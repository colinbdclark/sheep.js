...
(function () {
    ...
    var finishTestSequence = function (timing, spec) {
        // Total up all the samples, disregarding the outliers.
        for (var i = 0; i < spec.numReps; i++) {
            timing.total += timing.samples[i];
        }
        timing.total -= timing.max;
        timing.total -= timing.min;

        timing.avg = timing.total / (spec.numReps - 2);
